...
<template>
    <Head :title="title" />
    <keep-alive>
        <div class="min-h-screen bg-[#F5F6FB] flex relative">
            <!-- Left Sidebar -->
            <aside
                ref="leftSideBar"
                :class="[
                    'w-64 bg-white absolute md:relative z-50 transition-all duration-500 -left-full md:left-0  md:block me-4',
            ...
            <main :key="$page.url" class="flex-1 overflow-auto">
                <div class="p-4">
                    ...
                    <div class="flex gap-6 items-center justify-between mb-6">
                        <PanelLeft
                            ref="panelLeft"
                            @click="leftSideBarActive = !leftSideBarActive"
