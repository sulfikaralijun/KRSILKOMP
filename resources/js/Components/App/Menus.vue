    ...
    <nav class="mt-8 space-y-2">
        ...
        <div v-for="(item, index) in menuItems" :key="item.id">
            <div class="flex justify-between items-center w-full">
                ...
                <div v-else @click="toggleMenu(index)" :class="isActive(item)" class="menus cursor-pointer w-full">
                    <component :is="resolveIcon(item.icon)" class="w-5 h-5" />
                    <span>{{ item.name }}</span>
                    <LucideIcons.ChevronDown v-if="!menuState[index]" class="w-4 h-4 ml-auto" />
                    <LucideIcons.ChevronUp v-else class="w-4 h-4 ml-auto" />
                </div>
            </div>

            <div v-if="item.children && item.children.length && menuState[index]" class="ml-6 space-y-1 mt-2">
                <Link v-for="child in item.children" :key="child.id" :href="route(child.route)" :class="isActive(child)"
                    class="submenu">
...
const isActive = (item) => (route().current(item.route) ? 'active' : 'inactive');

function resolveIcon(iconName) {
    const formattedName = iconName;
    return LucideIcons[formattedName] || LucideIcons.LayoutGrid;
}
